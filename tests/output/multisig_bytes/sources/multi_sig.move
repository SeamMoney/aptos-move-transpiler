module 0x1::multi_sig {
    ...
    const E_OWNERS_REQUIRED: u64 = 266u64;
    const E_INVALID_NUMBER_OF_REQUIRED_CON: u64 = 267u64;
    const E_OWNER_NOT_UNIQUE: u64 = 268u64;
    const E_ONLY_VIA_MULTISIG: u64 = 269u64;
    ...
    public entry fun initialize(deployer: &signer, owners: vector<address>, num_confirmations_required: u256) {
        let (_resource_signer, signer_cap) = account::create_resource_account(deployer, b"multi_sig");
        assert!((vector::length(&owners) > 0), E_OWNERS_REQUIRED);
        assert!(((num_confirmations_required > 0) && (num_confirmations_required <= vector::length(&owners))), E_INVALID_NUMBER_OF_REQUIRED_CON);
        let i: u256 = 0;
        while ((i < (vector::length(&owners) as u256))) {
        ...
        move_to(deployer, MultiSigState { owners: vector::empty(), is_owner: table::new(), num_confirmations_required: num_confirmations_required, transactions: vector::empty(), is_confirmed: table::new(), nonce: 0, signer_cap: signer_cap });
    }
    ...
    public fun submit_transaction(account: &signer, to: address, value: u256, data: vector<u8>): u256 acquires MultiSigState {
        ...
        vector::push_back(&mut state.transactions, Transaction { to: to, value: value, data: data, executed: false, num_confirmations: 0 });
    ...
    public entry fun confirm_transaction(account: &signer, tx_index: u256) acquires MultiSigState {
        let state = borrow_global_mut<MultiSigState>(@0x1);
        assert!((signer::address_of(account) == state.owner), E_UNAUTHORIZED);
        assert!((tx_index < (vector::length(&state.transactions) as u256)), E_NOT_FOUND);
        assert!(!(*vector::borrow(&state.transactions, (tx_index as u64))).executed, E_TX_ALREADY_EXECUTED);
        assert!(!*table::borrow_with_default(table::borrow(&state.is_confirmed, tx_index), signer::address_of(account), &false), E_TX_ALREADY_CONFIRMED);
        let transaction: Transaction = *vector::borrow(&state.transactions, (tx_index as u64));
        transaction.num_confirmations += 1;
        if (!table::contains(&state.is_confirmed, tx_index)) {
            table::add(&mut state.is_confirmed, tx_index, table::new());
        };
    ...
    public entry fun revoke_confirmation(account: &signer, tx_index: u256) acquires MultiSigState {
        ...
        assert!((tx_index < (vector::length(&state.transactions) as u256)), E_NOT_FOUND);
        assert!(!(*vector::borrow(&state.transactions, (tx_index as u64))).executed, E_TX_ALREADY_EXECUTED);
        assert!(*table::borrow_with_default(table::borrow(&state.is_confirmed, tx_index), signer::address_of(account), &false), E_TX_NOT_CONFIRMED);
        let transaction: Transaction = *vector::borrow(&state.transactions, (tx_index as u64));
        transaction.num_confirmations -= 1;
        if (!table::contains(&state.is_confirmed, tx_index)) {
            table::add(&mut state.is_confirmed, tx_index, table::new());
    ...
    public entry fun remove_owner(account: &signer, owner: address) acquires MultiSigState {
        ...
        assert!(((vector::length(&state.owners) - 1) >= state.num_confirmations_required), E_UNAUTHORIZED);
        *table::borrow_mut_with_default(&mut state.is_owner, owner, false) = false;
        ...
        while ((i < (vector::length(&state.owners) as u256))) {
            if ((*vector::borrow(&state.owners, (i as u64)) == owner)) {
                *vector::borrow_mut(&mut state.owners, (i as u64)) = *vector::borrow(&state.owners, ((vector::length(&state.owners) - 1) as u64));
    ...
    public entry fun change_requirement(account: &signer, required: u256) acquires MultiSigState {
        let state = borrow_global_mut<MultiSigState>(@0x1);
        assert!((signer::address_of(account) == @0x1), E_ONLY_VIA_MULTISIG);
        assert!(((required > 0) && (required <= (vector::length(&state.owners) as u256))), E_INVALID_REQUIREMENT);
    ...
    #[view]
    public fun get_transaction_count(): u256 acquires MultiSigState {
        let state = borrow_global<MultiSigState>(@0x1);
        return vector::length(&state.transactions)
    }

    #[view]
    public fun get_transaction(tx_index: u256): (address, u256, vector<u8>, bool, u256) acquires MultiSigState {
        let state = borrow_global<MultiSigState>(@0x1);
        let _to = @0x0;
        let _value = 0u256;
        let _data = vector::empty();
        let _executed = false;
        let _num_confirmations = 0u256;
        let transaction: Transaction = *vector::borrow(&state.transactions, (tx_index as u64));
        return (transaction.to, transaction.value, transaction.data, transaction.executed, transaction.num_confirmations)
    }

    #[view]
    public fun get_pending_transactions(): vector<u256> acquires MultiSigState {
        let state = borrow_global<MultiSigState>(@0x1);
        let pending_count: u256 = 0;
        let i: u256 = 0;
    ...
    #[view]
    public fun is_transaction_confirmed_by(tx_index: u256, owner: address): bool acquires MultiSigState {
        let state = borrow_global<MultiSigState>(@0x1);
        return *table::borrow_with_default(table::borrow(&state.is_confirmed, tx_index), owner, &false)
    }

    #[view]
    public fun get_confirmations(tx_index: u256): vector<address> acquires MultiSigState {
        let state = borrow_global<MultiSigState>(@0x1);
        let count: u256 = 0;
        let i: u256 = 0;
        while ((i < (vector::length(&state.owners) as u256))) {
            if (*table::borrow_with_default(table::borrow(&state.is_confirmed, tx_index), *vector::borrow(&state.owners, (i as u64)), &false)) {
                count = (count + 1);
            };
            i = (i + 1);
        }
    ...
    public fun encode_add_owner(owner: address): vector<u8> {
        return bcs::to_bytes(string::utf8(b"addOwner(address)"), owner)
    }

    public fun encode_remove_owner(owner: address): vector<u8> {
        return bcs::to_bytes(string::utf8(b"removeOwner(address)"), owner)
    }

    public fun encode_change_requirement(required: u256): vector<u8> {
        return bcs::to_bytes(string::utf8(b"changeRequirement(uint256)"), required)
    }
}