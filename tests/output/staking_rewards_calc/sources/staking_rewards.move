module 0x1::staking_rewards {
    ...
    public fun reward_per_token(): u256 acquires StakingRewardsState {
        ...
        return (state.reward_per_token_stored + ((((((last_time_reward_applicable() - state.last_update_time)) * state.reward_rate) * 1000000000000000000) / state.total_staked)))
    }

    #[view]
    public fun earned(account: address): u256 acquires StakingRewardsState {
        let state = borrow_global<StakingRewardsState>(@0x1);
        return ((((*table::borrow_with_default(&state.staked_balance, account, &0u256) * ((reward_per_token() - *table::borrow_with_default(&state.user_reward_per_token_paid, account, &0u256)))) / 1000000000000000000)) + *table::borrow_with_default(&state.rewards, account, &0u256))
    }

    #[view]
    public fun get_reward_for_duration(): u256 acquires StakingRewardsState {
        let state = borrow_global<StakingRewardsState>(@0x1);
        return (state.reward_rate * state.rewards_duration)
    }
    ...
    public entry fun stake(account: &signer, amount: u256) acquires StakingRewardsState {
        ...
        if ((signer::address_of(account) != @0x0)) {
            *table::borrow_mut_with_default(&mut state.rewards, signer::address_of(account), 0u256) = earned(signer::address_of(account));
    ...
    public entry fun stake_for(account: &signer, user: address, amount: u256) acquires StakingRewardsState {
        ...
        state.last_update_time = last_time_reward_applicable();
        if ((user != @0x0)) {
            *table::borrow_mut_with_default(&mut state.rewards, user, 0u256) = earned(user);
            *table::borrow_mut_with_default(&mut state.user_reward_per_token_paid, user, 0u256) = state.reward_per_token_stored;
        };
        assert!((amount > 0), E_INVALID_AMOUNT);
    ...
    public entry fun exit(account: &signer) acquires StakingRewardsState {
        let state = borrow_global<StakingRewardsState>(@0x1);
    ...
    public entry fun notify_reward_amount(account: &signer, reward: u256) acquires StakingRewardsState {
        ...
        state.last_update_time = last_time_reward_applicable();
        if ((@0x0 != @0x0)) {
            *table::borrow_mut_with_default(&mut state.rewards, @0x0, 0u256) = earned(@0x0);
            *table::borrow_mut_with_default(&mut state.user_reward_per_token_paid, @0x0, 0u256) = state.reward_per_token_stored;
        };
