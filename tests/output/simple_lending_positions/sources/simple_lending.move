module 0x1::simple_lending {
    ...
    public entry fun liquidate(account: &signer, borrower: address, debt_asset: address, collateral_asset: address, debt_to_repay: u256) acquires SimpleLendingState {
        ...
        accrue_interest(debt_asset, state);
        accrue_interest(collateral_asset, state);
        let (unused0, unused1, unused2, shortfall) = get_account_liquidity(borrower);
        assert!((shortfall > 0), E_NOT_LIQUIDATABLE);
    ...
    public fun get_account_liquidity(user: address): (u256, u256, u256, u256) acquires SimpleLendingState {
        ...
        let shortfall = 0u256;
        let assets: vector<address> = *table::borrow_with_default(&state.user_assets, user, &vector::empty());
        let i: u256 = 0;
        while ((i < (vector::length(&assets) as u256))) {
            ...
            if ((position.borrow_balance > 0)) {
                let borrow_balance: u256 = borrow_balance_with_interest(user, asset, state);
                let borrow_value: u256 = (((borrow_balance * *table::borrow_with_default(&state.asset_prices, asset, &0u256))) / 1000000000000000000);
        ...
        return (total_collateral_value, total_borrow_value, available_borrow, shortfall)
    }
    ...
    public fun get_borrow_rate(asset: address): u256 acquires SimpleLendingState {
        ...
        let utilization: u256 = (((market.total_borrows * 1000000000000000000)) / market.total_deposits);
        if ((utilization <= KINK)) {
            return (BASE_RATE + (((utilization * MULTIPLIER)) / 1000000000000000000))
        } else {
            let normal_rate: u256 = (BASE_RATE + (((KINK * MULTIPLIER)) / 1000000000000000000));
            let excess_utilization: u256 = (utilization - KINK);
            return (normal_rate + (((excess_utilization * JUMP_MULTIPLIER)) / 1000000000000000000))
        };
    }
    ...
    public fun get_supply_rate(asset: address): u256 acquires SimpleLendingState {
        ...
        return (((borrow_rate * utilization)) / 1000000000000000000)
    }
    ...
    public(package) fun accrue_interest(asset: address, state: &mut SimpleLendingState) {
        ...
        let interest_accumulated: u256 = (((market.total_borrows * interest_factor)) / 1000000000000000000);
    ...
    public(package) fun borrow_balance_with_interest(user: address, asset: address, state: &SimpleLendingState): u256 {
        ...
        return (principal_times_index / position.borrow_index)
    }
}