module 0x1::event_test {
    ...
    public entry fun emit_transfer(account: &signer, to: address, value: u256) acquires EventTestState {
        let state = borrow_global_mut<EventTestState>(@0x1);
        event::emit(Transfer { from: signer::address_of(account), to: to, value: value });
    }
}