module 0x1::l_b_pair {
    ...
    public entry fun initialize(account: &signer, base_factor: u16, filter_period: u16, decay_period: u16, reduction_factor: u16, variable_fee_control: u32, protocol_share: u16, max_volatility_accumulator: u32, active_id: u32) {
        ...
        set_static_fee_parameters(update_id_reference(set_active_id(state.parameters, active_id)), base_factor, filter_period, decay_period, reduction_factor, variable_fee_control, protocol_share, max_volatility_accumulator, state);
    }
    ...
    public fun get_static_fee_parameters(): (u16, u16, u16, u16, u32, u16, u32) acquires LBPairState {
        ...
        base_factor = (get_base_factor(parameters) as u16);
        filter_period = (get_filter_period(parameters) as u16);
        decay_period = (get_decay_period(parameters) as u16);
        reduction_factor = (get_reduction_factor(parameters) as u16);
    ...
    public fun get_variable_fee_parameters(): (u32, u32, u32, u64) acquires LBPairState {
        ...
        let parameters: u256 = state.parameters;
        volatility_accumulator = (get_volatility_accumulator(parameters) as u32);
    ...
    public fun get_oracle_sample_at(lookup_timestamp: u64): (u64, u64, u64) acquires LBPairState {
        ...
        if ((time_of_last_update < lookup_timestamp)) {
            parameters = update_volatility_parameters(parameters, get_active_id(parameters), lookup_timestamp);
    ...
    public fun get_swap_in(amount_out: u128, swap_for_y: bool): (u128, u128, u128) acquires LBPairState {
        ...
        let id: u32 = get_active_id(parameters);
        ...
        while (true) {
            ...
            if ((bin_reserves > 0)) {
                ...
                let amount_in_without_fee: u128 = (if (swap_for_y) shift_div_round_up((amount_out_of_bin as u256), SCALE_OFFSET, price) else mul_shift_round_up((amount_out_of_bin as u256), price, SCALE_OFFSET) as u128);
                let total_fee: u128 = get_total_fee(parameters, bin_step);
                let fee_amount: u128 = get_fee_amount(amount_in_without_fee, total_fee);
    ...
    public fun get_swap_out(amount_in: u128, swap_for_y: bool): (u128, u128, u128) acquires LBPairState {
        ...
        let fee = 0u128;
        let amounts_in_left: u256 = encode(amount_in, swap_for_y);
        let parameters: u256 = state.parameters;
        let bin_step: u16 = bin_step();
        let id: u32 = get_active_id(parameters);
        parameters = update_references(parameters, (timestamp::now_seconds() as u256));
    ...
    public fun swap(account: &signer, swap_for_y: bool, to: address): u256 acquires LBPairState {
        ...
        let parameters: u256 = state.parameters;
        let bin_step: u16 = bin_step();
        let active_id: u32 = get_active_id(parameters);
        parameters = update_references(parameters, (timestamp::now_seconds() as u256));
        while (true) {
        ...
        parameters = update(state.oracle, parameters, active_id);
        state.parameters = set_active_id(parameters, active_id);
        if (swap_for_y_) {
            transfer_y(amounts_out, token_y(), to);
        } else {
            transfer_x(amounts_out, token_x(), to);
        };
        non_reentrant_after();
    ...
    public entry fun flash_loan(account: &signer, receiver: address, amounts: u256, data: vector<u8>) acquires LBPairState {
        ...
        event::emit(FlashLoan { arg0: signer::address_of(account), arg1: receiver, arg2: get_active_id(state.parameters), arg3: amounts, arg4: fees_received, arg5: fees_received });
        non_reentrant_after();
    ...
    public fun mint(account: &signer, to: address, liquidity_configs: vector<u256>, refund_to: address): (u256, u256, vector<u256>) acquires LBPairState {
        ...
        let reserves: u256 = state.reserves;
        amounts_received = received(reserves, token_x(), token_y());
    ...
    public entry fun force_decay(account: &signer) acquires LBPairState {
        ...
        only_factory();
        let parameters: u256 = state.parameters;
        state.parameters = update_volatility_reference(update_id_reference(parameters));
        event::emit(ForcedDecay { arg0: signer::address_of(account), arg1: get_id_reference(parameters), arg2: get_volatility_reference(parameters) });
        state.reentrancy_status = 1u8;
    }
    ...
    public(package) fun set_static_fee_parameters(account: &signer, parameters: u256, base_factor: u16, filter_period: u16, decay_period: u16, reduction_factor: u16, variable_fee_control: u32, protocol_share: u16, max_volatility_accumulator: u32, state: &mut LBPairState) {
        ...
        let bin_step: u16 = bin_step();
        let max_parameters: u256 = set_volatility_accumulator(parameters, max_volatility_accumulator);
        let total_fee: u256 = (get_base_fee(max_parameters, bin_step) + get_variable_fee(max_parameters, bin_step));
    ...
    public(package) fun update_bin(account: &signer, bin_step: u16, active_id: u32, id: u32, max_amounts_in_to_bin: u256, parameters: u256, state: &mut LBPairState): (u256, u256, u256) {
        ...
        let price: u256 = get_price_from_id(id, bin_step);
        let supply: u256 = total_supply(id);
        (shares, amounts_in) = get_shares_and_effective_amounts_in(bin_reserves, max_amounts_in_to_bin, price, supply);
        ...
        if ((id == active_id)) {
            ...
            if ((fees != 0)) {
                ...
                let bin_liquidity: u256 = get_liquidity((bin_reserves + (fees - protocol_c_fees)), price);
                shares = mul_div_round_down(user_liquidity, supply, bin_liquidity);
                parameters = update(state.oracle, parameters, id);
                state.parameters = parameters;
